...
\begin{itemize}
\item \color{orangei} \textbf{ggss-misc} \color{black} - zawiera szablony programu CMAKE oraz pliki nagłówkowe szeroko używane w projekcie
...
\begin{table}[]
\centering
\caption{Flagi użyte podczas procesu budowania za pomocą narzędzia CMake w zależności od wybranej konfiguracji. Tabela omawia tylko konfiguracje stosowane w projekcie (poza nimi dostępne są jeszcze trzy inne). \cite{CMakeOptions}}
\begin{tabular}{|c|c|}
\hline
...
\begin{lstlisting}[language=cmake, caption={Dodanie zewnętrznej biblioteki do projektu za pomocą narzędzia \textbf{CMAKE}}, label={lst:cmakeExLib}]
